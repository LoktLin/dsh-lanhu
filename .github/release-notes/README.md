...
- **CLI 不支持指定账号**（0.4.1 体检时发现）：`search` / `projects` 这类"只给 id"的命令只传 `{cookie}`，
  走**默认账号**，而别的团队属于别的账号（实测空天碳属 `kongtian`）→ 报 `30005`。
  工具的 `account` 参数是好的，**只有 CLI 缺**。一致性缺陷，补 `--account` 即可
  （判定逻辑在 `pickAccount` 里现成，不用新写）。

**攒着的小改进 —— 0.4.0 已全部实现**（保留在此作为"当时的判断"的记录，**不删**）：

- ~~**`lanhu_list_designs` 不认链接**~~ → **0.4.0 已做**：加 `url`（项目页链接也认），
  `projectId` 改为非必填但**两者都不给要明确报错**（不许静默给空表），显式 `projectId` 优先；CLI `designs --url` 同步。
  **实现时才发现**：不能用 `parseLanhuUrl` —— 它面向"**某一张稿**"、**没有 `image_id` 就抛错**，
  而项目页/列表页链接本来就没有 `image_id`（实测直接报"没找到 image_id"）。为此新增了 `parseProjectTarget`。
- ~~**清单里没有 `order`**~~ → **0.4.0 已做**：清单加「序」列，并在输出里写明"面板按它倒序、是滚动区，界面里只看到前几个不代表只有那几个"。
- ~~**可选 `withPages`**~~ → **0.4.0 已做**：**默认关**；纯函数 `countSitemapPages`（可离屏自检）；
  **单份失败只标 `?`**（原因在 `pages.reason`），绝不炸整张表；CLI `--with-pages` 同步。
- ~~**层级标题没标明是"路径"**~~ → **0.4.0 已做**：标题行与**每个页面标题**都标 `路径：`（两处都要 —— 只标一处时去掉另一处的变异不会红，已做变异测试）。
