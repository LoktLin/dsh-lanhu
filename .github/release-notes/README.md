...
3. **跑自检**：`node test/selfcheck.mjs` —— 530 项，**纯离线、秒级**，改完代码先跑它。
   ...
   顺手核对 README 里写死的项数（**530 / 16**）有没有过期。
...
**攒着的小改进（0.3.0 后记下，等下个真改动一起发 —— 不为它们单独发版本）**：

- **`lanhu_list_designs` 不认链接**：入参只有 `projectId`（必填）/`sector`，但它兄弟 `lanhu_list_product_documents` 收 `url`。
  结果"列出这个项目的设计稿"反而要调用方**先手拆 pid**（而这个插件对外的承诺是「链接里的 tid/pid/image_id 不用手拆」）。
  **这是一致性缺陷，不是新功能** —— 实测某项目 252 张设计稿，只能手拆 pid 才列得出来。
- **清单里没有 `order`**：`product_documents` 返回的 `order`（实测 2..10 连续）正好解释"界面上看到 3 个、接口给 9 个"这类疑问
  （界面是按 `order` 倒序的滚动面板）。加一列，AI 就能把「界面上看到的」与「接口给的」对上号。
- **可选 `withPages`**：清单里附上每份原型的**页面节点数 / 可读页数**（实测同项目里从 3 页到 219 页，差 20 倍）。
  现在要选对文档得逐份跑（18 个请求），做成开关（**默认关**，N 倍请求）能让 AI 一眼选对稿。
- **层级标题没标明是"路径"**：嵌套页面的标题会拼成 `A / B / C`，看起来像"把多页合并了"（我自己就这么误读过一次）。
  前面加个 `路径：` 即可。
