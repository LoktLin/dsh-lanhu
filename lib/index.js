...
const readDesignTool = tool({
  ...
  parameters: {
    ...
    format: {
      type: 'string',
      description: 'summary=紧凑文本（默认，含色板/字号/关键容器/文本层）；full=全量图层落盘 JSON 并返回路径；tokens=仅色板/字号/圆角统计',
      enum: ['summary', 'full', 'tokens'],
    },
    ...
    mapBox: { type: 'string', description: '配合 region：设计稿参照框 "x0,y0,x1,y1"（如九地市 Region 总 bbox）。与 toBox 同时给时，输出增加映射后的坐标列' },
...
const cookieSetTool = tool({
  ...
  description: '更新蓝湖 Cookie。**直接粘贴浏览器里复制的内容即可**：F12 → Network → 任意 lanhuapp.com 请求 → 右键 → Copy as cURL → 把整段贴进来（会自动解析出 Cookie，不用手工抠串）。也接受 "Cookie: ..." 原始请求头或裸 Cookie 串。写前用真实请求校验，再落盘到 ~/.dsh/lanhu/cookie（600）。',
  parameters: {
  ...
  async execute(args) {
    try {
      const r = await saveCookie(args.cookie, { dryRun: Boolean(args.dryRun) });
      const lines = [
