...
const listDesignsTool = tool({
  ...
  description: '列出某个项目下的全部设计稿（稿名 / 尺寸 / imageId）。',
  parameters: {
    projectId: { type: 'string', description: '项目 UUID', required: true },
  ...
  async execute(args) {
    try {
      const r = await listImages(args.projectId, { account: args.account });
      const lines = [`${r.projectName ?? args.projectId}：${r.images.length} 张设计稿`];
...
const listProductDocumentsTool = tool({
  ...
  description: '列出蓝湖项目下的**产品文档**（也叫原型 / PRD —— Axure 导出，`docType=axure`）。**这不是设计稿**：设计稿回答"什么颜色、几 px 圆角"，产品文档回答"业务规则、字段、跳转"。当用户给的是**原型链接**（URL 里带 `docType=axure`），或要看需求文档/PRD/原型、要定位某一步的业务规则时用它；顺带返回项目名/文件夹/创建者。设计稿请用 lanhu_list_designs。拿到 docId 后交给 lanhu_read_product_doc 读页面树与正文。',
  parameters: {
  ...
  async execute(args) {
    try {
      ...
      const lines = [
        ...
        '',
        '| # | 名称 | docId | 最新版本 | 版本数 | 更新时间 | 已替换 |',
        '|---|---|---|---|---|---|---|',
        ...listed.axureDocs.map((d, i) => `| ${i + 1} | ${d.name} | ${d.docId} | ${d.latestVersion ?? '—'} | ${d.lastVersionNum ?? '—'} | ${d.updateTime ?? '—'} | ${d.isReplaced ? '**是**' : '否'} |`),
        '',
...
const readProductDocTool = tool({
  ...
  description: '读蓝湖**产品文档（Axure 原型 / PRD）**的页面树与正文 —— 需求文档、原型交互、业务规则的来源。**不是设计稿**（色值/字号/圆角用 lanhu_read_design）。返回：页面树（层级/path/类型/pageId）+ 命中页的正文文本。**先不带 pageId 调一次看页面树**（一份原型常有上百个节点），再按 pageId（**跨版本稳定**）或 pageName 精确取正文。正文实测取自页面 HTML（data.js 里的原生控件多为空 —— 因为原型常以矢量/图片导出，只解析 data.js 会得出"这页没内容"的假结论）。',
  parameters: {
