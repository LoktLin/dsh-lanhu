...
## 0.4.0 —— 四条攒着的改进（2026-09-24）

> 这四条是 0.3.0 之后**主动记在「已知空缺」里攒着**的（不为它们单独发版本，等一个真改动一起做）。
> 其中第一条既是**新能力**也是**一致性修复**。
...
- **清单加 `order`**：`product_documents` 的 `order` 正好解释"界面只看到 3 个、接口给 9 个"这个疑问 ——
  蓝湖「文档」面板**按 `order` 倒序**显示且是**滚动区**。输出里加了这句话，AI 不会再以为插件读多了。
- **`withPages`（默认关）**：附上每份原型的**页面节点数 / 可读页数**。实测同一个项目里从 **3 页到 219 页（差 20 倍）**，
  有了它 AI 能**一眼选对文档**，不必逐份读一遍才发现拿错。代价是 **N 份 = N 次额外请求**（已写进描述与文档）；
  单份失败**只把该行标 `?` 并给原因，不让整张表崩**。
- **标题标明「路径」**：嵌套原型页的标题会拼成 `A / B / C`，**看起来像"把多页合并了"**（实测我自己就据此误报过一次）。
  现在统一走 `titleName()`，输出 `路径：A / B / C`。

### 体验优化

- **CLI 与工具两条链对齐**：`designs --url`、`product-docs --with-pages`（这个项目有过"CLI 是对的、只有工具链断了"的前例，反过来同样得防）。
- **判定放在源头**：`renderProductLayers` 传 `name` 时同时给 `nameIsPath: true`，下游 **4 个标题打印点**统一走 helper ——
  比"每处自己拼字符串"更不容易漏（第 4 个站点不会又忘）。
- 机器读的 JSON 也带 `nameIsPath` 字段（`name` 保持原值，展示格式不污染程序消费）。

### 问题修复

- **`parseLanhuUrl` 不能拿来列设计稿**：它面向"**某一张稿**"，**没有 `image_id` 就抛错**，而项目页链接本来就没有它
  （第一版真机直接撞上）。→ 新增 `parseProjectTarget`（面向"某个项目"），并把共用参数抽取抽出来；
  同时加**反向断言**：`parseLanhuUrl` **必须继续要求 `image_id`**，不许为迁就新用法被改宽容。
- **同一个 bug 出现两次**：列表渲染"**多一个空列**"（单元格带了前导 `|` 而行模板已收尾），
  工具与 CLI **各栽一次**（因为各写了一遍渲染）→ 抽成共用纯函数 `productDocsTable`，并断言表头/分隔/行列数一致。
- **一条断言是瞎的**：`路径：` 在标题行与每页标题**两处**都有，最初只断言了标题行 ——
  把「每页标题」去掉的变异**注入成功却 0 条红**（先确认注入有效才下的结论）。拆成两条后两处各自能红。

### 其他变更

- 自检 **530 → 564 项**；变异测试 7 处全部变红（含**源头标记**那条 —— 只测打印点的话，源头漏标记会让四个站点同时失灵却不报红）。
- 「已知空缺」里这四条**标记为 0.4.0 已做**（保留记录，不删）。
- `docs/原型样式.md` 新增「标题里的『路径：』是什么意思」；`docs/产品文档.md` 补 `order` 与 `withPages`。
- **未验证**：`renderFonts` 在原型链上是否真会被调用（目前 `format:'layers'` 走 `renderBlocks`）；
  `withPages` 的"单份失败 → `?`"分支只在合成夹具上验过（真机 9 份全成功）。
