...
## 0.4.4 —— 把 README 同步到 npm 页面（2026-09-24）

> 0.4.3 发布后才发现：**npm 页面上的「主文档」是按发布那一刻定格的** ——
> 之后改 GitHub 上的 README，npm 页面**不会跟着变**，只能再发一版。

### 体验优化

- **npm 页面顶部加三个徽章**（npm 版本 / 许可证 / Node 版本要求）—— 一眼能看出"这是个已发布的 npm 包"。
- README 的安装段**在 npm 页面上也生效了**（0.4.3 发的时候那段还没改）：
  ```bash
  dsh plugin --profile web add dsh-lanhu        # 从 npm（推荐使用者）
  dsh plugin --profile web add <plugin-dir>     # 从本仓库（要改代码时）
  ```
- 顺带记进发布清单：**改了 README 想让它出现在 npm 页面上，必须再发一版** ——
  README 是**跟着版本走**的，不是跟着仓库走。

## 0.4.3 —— 为发布到 npm 做准备（2026-09-24）

> 目标：让这个包**在 npm 上也是体面的**（别一点开就是一堆死链）。

### 体验优化

- **README 的文档链接改成 GitHub 绝对地址**（22 条）：`docs/*.md` / `CHANGELOG.md` / `.github/release-notes/*`
  原来是相对路径，**在 npmjs.com 上会 404**（那些文件按 `files` 不在包里，npm 也不给文件浏览）。
  改成绝对地址后 **GitHub 与 npm 两边都能点**。页内锚点与外链未动。
- **`package.json` 的 `files` 加 `docs/` 与 `CHANGELOG.md`**：文档随包走 ——
  对一个"给 AI 用的包"来说，读得到的文档本身就是能力的一部分。
  打包结果：**7 文件 / 146.5 kB → 14 文件 / 186.9 kB**。

### 其他变更

- **绊线加强**：README 链接检查原来只验**相对路径**。链接改绝对之后，剩下的相对链接没几条，
  这条断言会退化成**空跑（假绿）** —— 所以现在**两种形态都验**：GitHub 绝对链接被映射回仓库相对路径再查存在性，
  并加一条"**确实检查到了链接（不是空跑）**"（实测 10 条）。
- 发布准备核对（实测）：包名 **`dsh-lanhu` 在官方源可用**（404）· 产物**已脱敏**
  （公司名 / 账号别名 / 本机路径 / 真实 UUID 全 0）· 许可证 **MIT 一致** · 版本与 git tag 对应。

## 0.4.2 —— CLI 能指定账号了（+ 一个"看着成功却什么都没干"）（2026-09-24）

> 起因是 0.4.1 体检时记进「已知空缺」的那条：**工具能指定账号，CLI 不能**。

### 问题修复

- **CLI 补上 `--account`**：`parseArgv` 早就把 `--account` 解析成 `args.account`，但 **16 个命令里一个都没往下传**
  （实测：改动前 **0 个**命令透传，改动后 **13 个**）→ 它们永远走**默认账号**。
  真机后果：示例团队属账号 `demo`，默认是 `default-acct` → `search` / `projects` 报 **`code=30005`**，
  而同样的入参用**工具**就正常。现在 **13 个命令 / 15 个调用点**透传；3 个进**显式白名单**并注明理由
  （`cmdLog` 纯本地不走网络 · `cmdAccounts` 用 `--alias` 指定"操作哪个账号"，与"用哪个身份"语义不同 ·
  `cmdWho` 的职责本就是**跨账号判定**）。假账号 → **明确报错并列出已知账号**，退出码 1，**不静默回退默认**。
- **`search` 的人读输出丢结果**（顺带发现，比上面那条更糟）：`cmdSearch` 原先**只遍历 `r.images`** ——
  于是"命中的是 PRD / 项目而不是稿"的搜索变成**零输出 + 退出码 0**，即本项目最忌讳的**"看着跑成功但什么都没干"**。
  实测搜「某大屏」命中 **2 条 PRD / 0 张稿** → 什么都不打，而 `--json` 里 `prds:2`（数据明明在）。
  现在**三类都打** + 一行汇总；**三类全空时也明说"没有匹配"**。
...
- **`USAGE` 写清账号优先级**：`--cookie` > `LANHU_COOKIE` > `--account` > 默认账号。
  （这是在 `resolveCookie` 里挖到的：**显式 cookie 与环境变量会盖过 `--account`** —— 不写出来就是静默意外。）
- `docs/CLI与开发.md` 补 `--account` 与真实例子（30005 那条）。

### 其他变更

- **新增一条通用绊线**：**每个走网络的 `cmdXxx` 都必须把 `args.account` 传给它调的核心函数**
  （源码区间 + **字符串感知**扫描，带显式白名单）。工具链早就有"参数声明了但没传"的守卫，
  **CLI 没有 —— 这个缺口正是从那道缝里活下来的**。自检 **569 → 611 项**。
- **写这条守卫时自己踩了两个"假绿"、都被自己的守卫抓住**：
  ① 括号配平**从参数表的 `{` 就开始数** → `( args, cookie )` 一闭合就返回（只拿到 41 字符）→
  **所有命令被当成"不走网络"**（是"确实检查到了、不是空跑"那条断言判红的）；
  ② 补完后扫描名单**漏了 `whoIsIt` / `buildAccountIndex`** → **白名单成了摆设**
  （清空白名单 0 条红，变异测试抓出来的）。补全后清空白名单报 **2 条红**。

## 0.4.1 —— 结构性重构（**行为不许变**）（2026-09-24）

> 起因是一次代码体检。结论是**代码本身健康**（`fetch` 全带超时、`JSON.parse` 全在 try 里、
> 面板的 localStorage 有内存兜底、CLI 的命令本来就调同一套核心函数），
> 真正的问题只在**可维护性**上 —— 于是做一次重构，**不夹带任何功能**。

### 验证方式：黄金输出对比（这次的核心）

重构**之前**先采基线：**24 条 CLI 命令**（stdout/stderr/退出码）+ **19 个工具调用**（`text` 与**返回对象字段名集合**）。
重构后跑**同一个脚本**、归一化（只掩掉天然易变的位：输出目录路径 / ISO 时间戳 / 使用记录增长）、**逐字节 diff**。

**结果：44 份里 41 份逐字节相同。** 3 处差异全部可解释：

| 差异 | 内容 | 判定 |
|---|---|---|
| `read_product_doc`（两个用例） | **仅**字段表多了 `projectInfoError`（14→15）；`text` 长度 1954B / 2797B **前后一致** | 本次**唯一**被允许的输出变化（见下） |
| `MANIFEST.txt` | 仅字段数 14→15 | 同上 |

> 也先证明了**基线自身是确定的**：两次独立采集 → 归一化后 **0 差异**。
> （踩到一个坑：`slices` 的下载数**本身就抖动**（同代码连跑 11/12/**0**）→ 改成**重试到非退化**再比结构投影。）
...
- **拆长函数**：
  ...
  | 函数 | 前 | 后 |
  |---|---|---|
  | `main` | **351 行**（17 个 `case` 挤在一个 `switch`） | **26 行**（解析 + 分发 + 统一错误）+ **16 个 `cmdXxx`** + `CLI_COMMANDS` 命令表 |
  | `normalizeAxurePage` | **292 行** | **176 行**（遍历 + 编排）+ **纯函数 `axureNodeFields`**（每节点的几何/颜色/字体/描边/圆角/内边距） |
  | `compareBlockProps` | **173 行** | **21 行** + **6 个比较器**（`cmpRadius` / `cmpSize` / `cmpTextColor` / `cmpFontWeight` / `cmpFill` / `cmpBorder`） |

- **URL 解析入口收敛**：真的找到了一处重复 —— `parseProductUrl` **自己抄了一份**与 `lanhuUrlParams` 逐行相同的参数抽取循环。
  删掉后**全文件参数抽取循环只剩 1 处**。并写下"该用哪个"的两层说明：
  底层 `lanhuUrlParams`（只抽参数不校验）；上层按语义分 `parseLanhuUrl`（**必须有 `image_id`**，某一张稿）/
  `parseProjectTarget`（**不要求** `image_id`，某个项目）/ `parseProductUrl`（一份原型文档）；`resolveTarget` 只做胶水。
...
- **删掉死导出 `PW_INSTALL_HINT`**。删之前先查清它是**已发布 5 个版本的别名**（不是"从没被用过的预留"，
  也不是"被重命名掉的旧名" —— 证据是两个名字**自首个公开版就并存**）。

### 其他变更

- 自检 **564 → 569 项**；变异测试 3 处全红（去掉两处留痕各 1 条、`tryProjectInfo` 把原因吞成 `null` 红 1 条）。
- **过程中自检抓到一个真回归**：重写 `main` 时漏了 `console.log(USAGE)` → 无参调用变成**零输出**
  （正是本项目最忌讳的"看着跑成功但什么都没干"），当场判红、补回才绿。
- 新记入「已知空缺」：**CLI 不支持指定账号**（`search` / `projects` 这类只给 id 的命令只传 cookie，
  走**默认账号**，非默认账号会报 `30005`；而工具的 `account` 参数是好的）。**一致性缺陷，下个版本补 `--account`。**
